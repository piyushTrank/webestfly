...
const CancelRefundPdf = ({ data }) => {
  ...
  const renderPassenger = () => {
    return filterArr.map((el) => (
      <Text style={{ ...styles.txt, ...styles.boldTxt }}>{el}</Text>
    ));
  };
      ...
      <Document>
        ...
        >
          <View style={styles.section}>
            ...
            <Text style={styles.txt}>
              There will be 1 charge appearing on your card statement of{" "}
              <Text style={styles.boldTxt}>${data.priceField2} USD</Text> as{" "}
            ...
            <View style={styles.cardDetWrap}>
              ...
              <Text style={{ ...styles.txt, fontWeight: 500 }}>
                Refund Amount -{" "}
                <Text style={styles.boldTxt}>${data.refundAmt} USD</Text>
              </Text>
              <Text style={{ ...styles.txt, fontWeight: 500 }}>
                <Text style={styles.boldTxt}>${data.refundAmtPrice1} USD</Text>
                {data.refundAmtPrice2 !== "" ? (
                  <Text style={styles.boldTxt}>
                    + ${data.refundAmtPrice2} USD
                  </Text>
                ) : (
                  ""
                )}{" "}
                Taxes And Fees Each Passenger will be charged by the
                consolidation desk i.e {data.bookedThrough}
              </Text>
              <Text style={{ ...styles.txt, fontWeight: 500 }}>
                A few notes about the airline refund: It takes upto 1-2 billing
                cycles or more to reflect on your account statement.
              </Text>
              <Text style={styles.txt}>
                When the refund is processed a refund tracking number is shared
                with the passengers over the email.
              </Text>
              <Text style={styles.txt}>
                Please be advised that the refund desk can only be reached at
                the Customer Care team{" "}
                <Text style={{ ...styles.boldTxt, color: "#1B5E9B" }}>
                  {data.phoneNumber}
                </Text>
                .
              </Text>
...
export default CancelRefundPdf;
